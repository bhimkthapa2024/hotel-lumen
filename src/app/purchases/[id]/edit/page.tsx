...
export default function EditPurchasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    try {
      ...
      const existingPurchase = purchasesData.find((p: Purchase) => p.id === id);
      if (existingPurchase) {
        setFormData({
          date: existingPurchase.date,
          invoiceNumber: existingPurchase.invoiceNumber,
          voucherType: existingPurchase.voucherType || 'PURCHASE VOUCHER',
          supplierId: existingPurchase.supplierId,
          expenseHeadId: existingPurchase.expenseHeadId,
          subLedger: existingPurchase.subLedger || '',
          exemptedValue: existingPurchase.exemptedValue?.toString() || '',
          purchaseValue: existingPurchase.purchaseValue?.toString() || '',
          isTdsApplicable: existingPurchase.tdsAmount > 0,
  ...
  return (
    <div className="container" style={{ maxWidth: '1000px' }}>
      ...
      {loading && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255,255,255,0.8)', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading existing purchase...</div>}

      <div className="glass-panel" style={{ padding: '3rem', borderRadius: 'var(--radius-lg)', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.05), 0 8px 10px -6px rgba(0, 0, 0, 0.01)', position: 'relative' }}>
